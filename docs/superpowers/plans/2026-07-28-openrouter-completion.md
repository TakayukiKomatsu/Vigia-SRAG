...
- [x] **Step 1: Write failing streaming contract tests**

Create fake OpenAI-client chunks and assert that the adapter calls Chat Completions with `stream=True`, a JSON-schema `response_format`, an evidence-ID allowlist, deterministic temperature, bounded output, provider parameter enforcement, and reasoning disabled. Assert that streamed content becomes a `CommentaryResult` with `requested_model="openrouter/free"`, the actual `chunk.model`, and grounded claims.
...
- [x] **Step 2: Write failing error-path tests**

Cover one retry for transient provider errors and invalid/incomplete structured output; fail closed after the second failure; reject missing model metadata, non-`stop` finish reason, unknown evidence IDs, or malformed JSON after the bounded retry; fail immediately for non-retryable SDK errors.
...
stream = self._client.chat.completions.create(
    ...
    extra_body={"provider": {"require_parameters": True}, "reasoning": {"effort": "none", "exclude": True}},
)
```

Collect only textual deltas, require terminal `finish_reason == "stop"` and served-model metadata, normalize only omitted provider-generated claim IDs deterministically, validate `CommentaryClaims`, and revalidate evidence references before returning. Allow one bounded retry for transient or invalid/incomplete output failures.
...
- [x] **Step 1: Rewrite the golden fixture test to preserve PNI scope**
...
- [x] **Step 2: Add a failing national-metric scope test**
...
- [x] **Step 3: Run governance tests and observe failure**
...
- [x] **Step 4: Implement the two-set invariant**
...
- [x] **Step 1: Add failing provider-key tests**
...
- [x] **Step 2: Add failing adapter-selection tests**
...
- [x] **Step 3: Run CLI tests and observe failure**
...
- [x] **Step 5: Update the environment template**
...
- [x] **Step 6: Run CLI and adapter tests**

Run: `uv run pytest tests/test_cli.py tests/test_commentary.py -q`

Expected: all tests pass.

- [x] **Step 7: Commit the CLI slice**
...
- [x] **Step 1: Update provider and PNI contracts**
...
- [x] **Step 2: Update acceptance scenarios and traceability**
...
- [x] **Step 3: Update README and sanitized sample**
...
- [x] **Step 5: Run the real OpenRouter smoke**
...
- [x] **Step 6: Update task ledgers truthfully**
...
- [x] **Step 7: Commit closeout artifacts**
