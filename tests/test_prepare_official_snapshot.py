...
def _sivep_bytes() -> bytes:
    fields = [
        "NU_NOTIFIC", "DT_NOTIFIC", "DT_SIN_PRI", "HOSPITAL", "DT_INTERNA", "SG_UF_INTE",
        "UTI", "DT_ENTUTI", "DT_SAIDUTI", "EVOLUCAO", "DT_EVOLUCA", "DT_ENCERRA", "DT_DIGITA", "SG_UF",
    ]
    row = ["fixture-1", "2026-07-02", "2026-07-01", "1", "2026-07-03", "SP", "2", "", "", "1", "2026-07-05", "2026-07-05", "2026-07-02T00:00:00.000Z", "SP"]
    return (";".join(fields) + "\n" + ";".join(row) + "\n").encode()
...
def _ods(path: Path) -> None:
    ...
    for values in (("LOCAL", "POPULAÇÃO ESTIMADA"), ("Brasil", "213.421.037")):
        row = table.TableRow()
        for value in values:
...
def _source(base: acquire.OfficialSource, *, payload: bytes, relative_path: str) -> acquire.OfficialSource:
    return replace(
        base,
...
def _record(source: acquire.OfficialSource, payload: bytes, *, rows: int | None = None) -> dict[str, object]:
    return {
        "key": source.key, "family": source.family, "identifier": source.identifier,
        "status": "verified", "retrieved_at": "2026-07-27T00:00:00Z",
        ...
        "encoding": source.encoding, "expected_sha256": source.expected_sha256,
        ...
        "watermark": source.watermark, "optional": source.optional,
        "raw_sha256": hashlib.sha256(payload).hexdigest(), "size_bytes": len(payload),
        "data_rows": rows,
    }


def test_acquisition_records_complete_provenance_and_independent_optional_failure(tmp_path: Path) -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        return httpx.Response(200 if request.url.path == "/sivep.csv" else 404, content=sivep_payload, request=request)
...
def test_prepare_sivep_only_publishes_empty_supporting_tables_and_ineligibility(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    sivep_payload = _sivep_bytes()
    ...
    acquisition.write_text(json.dumps({"schema_version": "1.0", "sources": [
        _record(sivep, sivep_payload, rows=1), {"key": "ibge", "status": "unavailable"}
    ]}), encoding="utf-8")
    published = prepare.prepare_snapshot(acquisition_path=acquisition, sivep_csv=sivep_path, ibge_ods=None, output_root=tmp_path / "snapshots", snapshot_id="official-fixture", as_of=dt.date(2026, 7, 26), generated_at=dt.datetime(2026, 7, 27, tzinfo=dt.UTC))
    ...
    provenance_sidecar = (published / "official-provenance.json.sha256").read_text(encoding="ascii").strip()
    manifest = json.loads((published / "manifest.json").read_text())
    assert evidence["golden_eligible"] is False
    assert evidence["metric_states"]["mortality_per_100k"] == "unavailable"
    assert evidence["metric_states"]["icu_pressure"] == "unavailable"
...
def test_prepare_ignores_supplied_ibge_path_when_sidecar_is_unavailable(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    sivep_payload = _sivep_bytes()
    sivep_path = tmp_path / "INFLUD26.csv"
    sivep_path.write_bytes(sivep_payload)
    sivep = _source(acquire.SIVEP_SOURCE, payload=sivep_payload, relative_path="sivep/INFLUD26.csv")
...
def test_prepare_rejects_hash_mismatch_without_network(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    acquisition = tmp_path / "acquisition.json"
    acquisition.write_text(json.dumps({"schema_version": "1.0", "sources": [_record(source, _sivep_bytes(), rows=1)]}), encoding="utf-8")
    with pytest.raises(prepare.PreparationError, match="sivep_hash_mismatch"):
        prepare.prepare_snapshot(acquisition_path=acquisition, sivep_csv=csv_path, ibge_ods=None, output_root=tmp_path / "snapshots", snapshot_id="bad", as_of=dt.date(2026, 7, 26))


def test_prepare_with_independently_attested_ibge_ods(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    sivep_payload = _sivep_bytes()
    ...
    _ods(ods_path)
    sivep = _source(acquire.SIVEP_SOURCE, payload=sivep_payload, relative_path="sivep.csv")
    ibge = _source(acquire.IBGE_SOURCE, payload=ods_path.read_bytes(), relative_path="population.ods")
    monkeypatch.setattr(prepare, "SIVEP_SOURCE", sivep)
    ...
    acquisition.write_text(json.dumps({"schema_version": "1.0", "sources": [
        _record(sivep, sivep_payload, rows=1), ibge_record
    ]}), encoding="utf-8")
    published = prepare.prepare_snapshot(acquisition_path=acquisition, sivep_csv=sivep_path, ibge_ods=ods_path, output_root=tmp_path / "snapshots", snapshot_id="with-ibge", as_of=dt.date(2026, 7, 26), generated_at=dt.datetime(2026, 7, 27, tzinfo=dt.UTC))
    evidence = json.loads((published / "official-preparation.json").read_text())
    manifest = json.loads((published / "manifest.json").read_text())
...
def test_prepare_rejects_tampered_immutable_sidecar_fields(
    ...
    acquisition = tmp_path / "acquisition.json"
    acquisition.write_text(json.dumps({"schema_version": "1.0", "sources": [record]}), encoding="utf-8")
    with pytest.raises(prepare.PreparationError, match="sivep_immutable_metadata_mismatch"):
        prepare.prepare_snapshot(acquisition_path=acquisition, sivep_csv=csv_path, ibge_ods=None, output_root=tmp_path / "snapshots", snapshot_id="tampered", as_of=dt.date(2026, 7, 26))


def test_raw_inputs_and_snapshots_are_ignored_and_untracked() -> None:
    root = Path(__file__).parents[1]
    tracked = subprocess.run(
        ["git", "ls-files", "data/raw", "data/snapshots"], cwd=root, check=True,
    ...
    for path in ("data/raw/probe.csv", "data/snapshots/probe.duckdb"):
        ...
        untracked = subprocess.run(["git", "ls-files", "--error-unmatch", path], cwd=root).returncode
        assert ignored == 0
        assert untracked != 0
